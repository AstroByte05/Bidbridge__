...
TaskCard: (task) => {
        // This will log the task data to your browser's developer console.
...
async handleDeleteTask() {
    ...
    async handlePostTask(form) {
        // *** FIX: Collect form data directly to ensure correct types ***
        const taskData = {
            ...
            is_urgent: form.querySelector('#task-urgent').checked // This will be a true/false boolean
        };
        ...
        try {
            ...
            ui.closeModal();
            // We'll reload the dashboard to ensure the new task is displayed correctly.
            // This is a simple way to handle the state update.
            window.location.hash = '#dashboard'; 
            app.loadDashboard(); // Manually trigger a refresh
        } catch (error) {
            showNotification(error.message, true);
        }
    },
    ...
    subscribeToAllTasks() {
        state.taskSubscription = supabase.channel('public:tasks')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'tasks' }, payload => {
                // When any task changes, simply reload the dashboard for simplicity
    ...
    subscribeToBids(taskId) {
        state.bidSubscription = supabase.channel(`bids:${taskId}`)
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'bids', filter: `task_id=eq.${taskId}` }, payload => {
                // When a new bid comes in, reload the task detail view
